...
In addition to those, which are "React's faults", there are also just more allocations by default simply because there is no differentiation between initialization (like you know, `constructor()`) and rendering, so people keep doing allocations like this one:
