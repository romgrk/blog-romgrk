...
One example of ergonomic error handling: `Result` being a Functor means we could implement it as a class with a `.map` method, so we could `result.map(n => n * 2)` the value inside a `Result` the same way we can operate on values inside an `Array` (`Ok` would apply the mapping function, while mapping an `Err` would be a no-op). Other methods from FP & category theory would also be applicable, e.g. `.flatMap()` or `.fold()`, which in turn allow new expressive ways to write code:
