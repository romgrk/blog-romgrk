...
It's an issue I see frequently in even popular library code, for example let's take this example from `react-query`'s documentation:
...
<div class="row w-full">
  <img width="50%" src={exampleSuccess.src} />
  <img width="50%" src={exampleError.src} />
</div>
...
Proper error handling was pioneered by monadic languages such as Haskell, which has [the `Either` monad](https://hackage.haskell.org/package/base-4.20.0.1/docs/Data-Either.html) and its two subtypes, `Left` and `Right`. A more modern (and pragmatic) approach exists in Rust, which called the type `Result` and its two subtypes `Ok` and `Err`. Powerful type-systems are an incredibly useful tool, and we should be using it to ensure that we can't make silly mistakes like forgetting to check if a value is loading.
