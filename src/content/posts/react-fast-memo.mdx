...
```javascript
export function fastCompareForReactProps(a, b) {
  let aLength = 0;
  let bLength = 0;
...
```jsonc
{
  ...
  // Does not include ⚠️ assumptions
  ...
  // Does include ⚠️ assumptions
  "romgrk-fastCompareForReactProps:equal":   { t: 515,     stddev: 7.48 },
  "romgrk-fastCompareForReactProps:unequal": { t: 445.33,  stddev: 1.24 },
}
```

<sup>1</sup> Alright ok but we do need to admit that we've optimized for "react props", not for general correctness. We
return `true` for `equals({ a: undefined }, {})`, which works for us but not for a generic compare function. We do have
`fastCompare` which is still the fastest and 1.5x faster than react if we only consider candidates that use safe
