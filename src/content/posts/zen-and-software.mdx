...
I don't want to have to argue if the `{` should be after the `if` or on a line of its own. `eslint` can do that well enough. But beauty goes beyond `prettier`.

#### Spacing for clarity

And if the last example I went for using more vertical space, the opposite can also improve readability.
