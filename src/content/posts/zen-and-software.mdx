...
```javascript
const hasUninitialized = items.some(i => i.status === Status.Uninitialized)
const hasReady = items.some(i => i.status === Status.Ready)
...
I don't want to have to argue if the `{` should be after the `if` or on a line of its own. `eslint` can do that well enough, no need to let `prettier` destroy beauty.
