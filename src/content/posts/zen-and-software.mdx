...
Now yes, typescript can only help you so much, and you'll sometimes need to add runtime validation checks. That's ok. But there's only ever two places where there should be runtime validation checks: user input and network data. Eveything else is part of a machine that is fully in your control. The machine can accept fuzzy invalid user-input (and fail appropriately), but it should not contain any fuzziness in its insides. No loose screw, no duct tape. Validate your inputs, and then establish ruthlessly strict constraints for how the inside of the machine works.
...
// This is our component's dimensions.
// It's calculated on the first render, so we don't have a value for it yet.
// Let it be null for now.
let rootDimensions = null as Dimension | null
```

It might not be immediately apparent, but the consequence of picking `null` as the empty value condemns every single subsequent section of code that uses `rootDimensions` to add null checks and default values:
...
Whereas maybe initializing the dimensions with a monomorphic value negates the need for any later defensive check:
...
<Aside title="A note on performance">
  For those who spotted that "monomorphic" also ties into my post on performance, yes, having a monormphic value not only helps readability but also greatly improves performance: [Optimizing JS: Avoid different shapes](./optimizing-javascript#2-avoid-different-shapes).
</Aside>
...
Call me crazy but I always sort code lines this way, and I call these pyramids. Tell me, which one of these blocks seems easier to read for you?

<div class='code-blocks'>

```javascript
const hasUnitialized = items.some(i => i.status === Status.Unitialized)
...
const hasCompleted = items.some(i => i.status === Status.Completed)
```
...
Although the names sounds more natural when pronounced out loud for the first block, they're much easier to read as a whole in the second one.
...
Which of those block would you prefer to code review? The auto-formatted one, or the manually formatted one? For all their benefits, full-code formatters like `prettier` have gotten many programmers convinced that even talking about formatting and alignment is a negative, best left to machines to deal with. Code is not just about machines. It's about humans. I don't want to have to argue if the `{` should be after the `if` or on a line of its own or have to remind people about adding spaces around their `+` operators. `eslint` can do that well enough. But beauty goes beyond `prettier`.
...
In fact, if features are an asset, code is a liability. A general rule, and I think everyone will agree with me, is that the more code you have, the more bugs you have. So a software product in theory we should aim to have as many features as possible, while having as little code as possible. That's why one of my favourite activities as a programmer is deleting large sections of code.

In practice, in the front-end world this manifest itself as implementing features using browser APIs and CSS instead of re-building existing features in javascript (please no more stupidity like CSS-in-JS ever again).
