...
This point is actually implicit in the one above, but I'll expand on the subject. Retrieving memory from the RAM is *slow*. To speed up things, computers will fetch more than the memory asked by the program, because usually if you have asked for a region of memory, there's a pretty good chance that you'll be interested in the region that comes right after. CPUs also have various memory caches that are faster than the RAM: L1, L2, L3. Those are faster, but also smaller, so only a small subset of working memory is kept there, and it's written back to the main RAM only when there's not enough room in the L caches and space needs to be cleared for new incoming memory.
...
const points = new Array(8 * KB)
for (let i = 0; i < points.length; i++) {
  points[i] = { x: 5, y: 10 }
}

// arrays of numbers: these are compact in memory
const xs = new Int32Array(8 * KB).map(() => 5)
const ys = new Int32Array(8 * KB).map(() => 10)
```
