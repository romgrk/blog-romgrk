...
I'm not going to explain much here as most of this is already explained by sections 4 and 5. But you should know that engines represent small objects as "shapes", as we've seen in section 2. However, when the shape grows too large, the engine has no choice but to use a regular hashmap (same principle as `Map`). Let's see how it behaves with this map of some users indexed by their ID.
...
```javascript
// 1. [] access
let _ = 0; Object.keys(byId).forEach(id => { _ += byId[id].id })
```

```javascript
// 2. direct access
let _ = 0; Object.values(byId).forEach(user => { _ += user.id })
```
...
I tested the case above with a reasonable 1,000 entries object, but the performance keeps degrading as the size of the object grows:
...
As demonstrated above, avoid having to frequently index into the object. Prefer turning the object into an array beforehand. Organize your data to have the ID on the model, not just on the hashmap.
