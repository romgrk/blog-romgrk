...
As you can see, the difference can be significant. The difference isn't necessarily due to the `strcmp` cost as engines can sometimes use a string pool and compare by reference, but it can also be because integers are usually passed by value in JS engines, whereas strings are always passed as pointers, and memory accesses are expensive (see section 5). In string heavy code, this can have a huge impact.
...
If you've made it this far, consider visiting [The Castle](/castle).
