...
The proxy benchmark is particularly brutal on V8 at the moment. Last time I checked, proxy objects were always falling back from the JIT tiers to the interpreter, seeing from those results it might still be the case.
...
let result = 0
for (let i = 0; i < 100_000; i++) { result = result + point.x }
```
...
let result = 0
for (let i = 0; i < 100_000; i++) { result = result + x }
```
