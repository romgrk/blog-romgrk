...
Whereas if you have classNames `glorb` and `obul`, you again simply `clsx` and you're done. You can expose them & export them anywhere, and it works anywhere. Classes are native to the ecosystem and everyone understands them.
...
Because I'm sure I'll get comments about it, I'm of course not arguing that you shouldn't encapsulate components if they form the base of your app's design language. If you need to define commmon elements that are "style-only" but that still represent a different logical block, say `Card` vs `Box`, then sure go ahead. When you open a render function with `<Box>...</Box>` and `<Card>...</Card>`, you'll still know that nothing funky is going on because those become the common way to write `div` or `section` in your app and that's fine. It doesn't negates the fact that using styled-components everywhere is going to shit on the readability and reusability of your codebase. Styled components are a net negative in nearly all cases.
